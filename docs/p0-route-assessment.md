...
## 5. 2026-09-21 Import-first 实现决策

由于当前只有 Ubuntu 环境，项目已选择 **C：离线导入优先** 作为当前 V1 的实现路线：先完成与微信 Runtime 解耦的归档核心，再等待合法真实导入包或外部 Windows Agent 进行来源适配验证。

这是一项“实现路线”决策，不代表 P0 的真实数据验收已经完成。P0-06～P0-11 继续保持未完成；Linux Runtime 仍不能作为已验证的消息 Adapter。Phase 1 仅允许在合成 fixture 上推进，不得据此宣称真实微信同步可用。
