...
## 5. 2026-09-21 历史实现决策（已由产品确认更新）

2026-09-21 曾因只有 Ubuntu 环境，暂时选择 **C：离线导入优先** 来推进归档核心。这一决定只用于解除核心开发对真实来源的阻塞，并不代表用户认可手动导入作为最终产品流程。

## 6. 2026-09-22 产品主流程确认

用户已明确最终期望：

1. 添加账号后展示微信扫码入口；
2. 用户扫码并确认登录；
3. 系统检测到登录成功后自动创建并执行首次全量归档任务，不要求用户再点一次“开始同步”；
4. 后续如登录失效，增量任务进入 `waiting_qr`；扫码恢复登录后自动续跑当前任务；
5. 首次归档完成后，不做定时同步。以后只在用户点击“立即同步”时执行增量归档。

因此，Import-first 现有实现降级为归档核心测试、合成演示和兼容输入路径；它不是用户主流程。Phase 0 的最高优先级恢复为验证扫码 Runtime 登录后能否稳定、只读地获得联系人、会话、消息和媒体。P0-06～P0-11 仍未完成，不能宣称真实扫码同步可用。

当前 Ubuntu 环境可继续实现 Runtime 状态机、首次同步自动触发和 API/UI 接口，但端到端验收仍取决于真实来源读取 Adapter。不得用模拟登录事件或合成数据替代该验收。
