...
本报告是 Phase 0 的证据记录。Runtime 启动、扫码操作和源数据目录已在当前主机观察；程序化登录态检测、真实消息库读取、数据库解密和媒体关联仍未验证。扫码操作本身不等于消息归档成功。
...
| 目标数据读取方式 | 20 个 `.db` 中只有 1 个可只读打开；未发现明显消息/联系人/会话表 | 阻塞 |

因此 P0-01、P0-03、P0-04、P0-05 和 P0-12 已有可复核证据。P0-04 的结果是“登录资料保留，但重启后需要手机再次完成登录”；P0-06 仍未满足稳定、可维护的读取条件。P0-02、P0-06 至 P0-11 尚未满足退出条件。
...
### 3.7 2026-09-22 扫码后只读复核

用户在固定 Compose Runtime 的 KasmVNC 页面完成扫码确认后，本轮只检查 Runtime 进程状态、HTTP 状态和源目录文件元数据；没有读取聊天正文、数据库业务行或媒体内容。

```text
Runtime：固定 amd64 digest，容器运行中
微信版本：4.1.13.23
KasmVNC：HTTP 200
微信进程：可观察到 wechat / WeChatAppEx 进程
wechat-ctl status：只报告 installed/phase/version，不提供登录态字段
/config/xwechat_files：274 个文件，165,875,251 bytes
其中 *.db：20 个；标准 SQLite 文件头 1 个，其他/未知格式 19 个
只读 sqlite_master 探针：1 个数据库可打开，共 1 张表；无表名匹配常见 message/contact/conversation/chat 关键字
```

文件头和 SQLite schema 检查均使用只读方式，不输出文件名、表名或行值。该结果说明当前唯一可用标准 SQLite 库没有显而易见的消息业务表；它不证明其余 19 个文件一定加密，也没有确定消息数据库的位置。`wechat-ctl status` 不是登录态检查接口，扫码成功由用户确认，但 Runtime 尚不能向 Archive 提供可订阅的登录成功事件。P0-02 的隔离账号记录也仍需在仓库外按模板完成/确认。

### 3.8 2026-09-22 登录后密钥扫描与会话读取探针

用户在手机侧完成本次登录确认后，微信进入主界面。本轮仅为触发本地数据加载而打开一个会话；没有输入、发送、回复或修改任何消息。随后使用可替换候选 `wx-cli 0.3.0` 执行强制密钥扫描和会话枚举，聊天内容及密钥均未写入仓库或输出到本报告。
...
候选密钥：0
提取出的数据库密钥：0
会话枚举：失败，无法解密 session.db
真实会话/消息行：未读取
真实消息导入：未发生
消息发送/回复：未执行
```

结论：在微信已登录并打开会话的条件下，公开 npm 包 `@jackwener/wx-cli 0.3.0` 的当前 Linux 内存扫描仍无法为此 Runtime 找到密钥，因此该候选不能用于真实归档，也不能作为本项目依赖。登录成功、存在微信进程或扫描过内存都不构成消息同步证据。P0-06、P0-07 和真实消息同步验收继续未完成；在获得可复核且兼容当前微信版本的只读取钥方式前，不实现绑定该数据库格式的解密 Adapter。
...
P0 必须使用专用隔离测试账号，不使用用户最重要的主账号。仓库不得提交二维码截图、登录凭据、微信数据库、聊天正文或未脱敏媒体。测试账号信息只保存在本地受控记录中，不进入 Git。用户已确认扫码操作，但隔离账号的本地记录/属性仍需补齐，P0-02 暂不勾选。
...
`LoginKeyInfoTable` 只检查了列定义，没有读取 `key_info_data` 等任何行值。当前看到的可读数据库不是聊天消息归档库；其余文件可能是加密数据库或专有格式，但尚未获得合法、稳定的读取方式。在没有完成数据库识别和读取验证前，不实现猜测性的解密、表结构解析或消息导入代码。

2026-09-22 在用户确认扫码后再次以 `mode=ro&immutable=1` 检查 `/config/xwechat_files` 的 20 个 `.db` 文件：1 个可读取 SQLite schema，另外 19 个不能被系统 SQLite 识别；唯一可读库只有 1 张表，表名不匹配常见消息/联系人/会话关键字。未查询任何表行。当天目录计数与早期快照略有差异，因此本次原始路径和文件名不入库，只保留聚合统计；这些检查仍不能判定 19 个文件的加密算法或消息结构。
...
| P0-12 | 确认扫码主流程及来源路线边界 | 产品决策已完成；真实来源仍待验证 | 扫码成功自动首次归档、后续手动增量；见路线评估 §6 |

## 7. 当前路线判断

产品主流程已确定为“首次扫码登录成功后自动全量归档，之后手动增量，不做定时同步”；技术来源仍待验证：
...
- 离线导入：仅用于归档核心测试/演示和兼容 Adapter 输入，不作为产品扫码主流程。

在 P0-06～P0-11 获得真实数据证据前，不宣称扫码自动同步可用；归档核心已有离线测试实现，与真实来源验收分开记录。
